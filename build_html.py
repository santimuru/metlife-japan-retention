...
.article {{ max-width: 820px; margin: 0 auto; padding: var(--space-12) var(--space-8) var(--space-16); }}
...
.sec {{ margin-top: 3.4rem; padding-top: 1.8rem; border-top: 1px solid var(--border); }}
.sec > h2 {{ font-size: 1.4rem; font-weight: 600; letter-spacing: -.01em; color: var(--text); margin: 0 0 .3rem; }}
...
.seg-card > h3 {{ grid-column: 1 / -1; font-size: .98rem; font-weight: 600; gap: .5rem; margin: 0; }}
.seg-card > .seg-stat {{ display: flex; justify-content: space-between; gap: .6rem; font-size: .86rem; }}
.seg-card .seg-stat .k {{ color: var(--muted); }}
.seg-card > .note {{ grid-column: 1 / -1; font-size: .9rem !important; line-height: 1.5;
...
<article class="article">
  ...
  <section class="sec">
    ...
    <p>Because the scores rank well, risk is concentrated. Contacting the riskiest 20% of the book reaches {cap2} of all
      the lapses that are going to happen, with a top-decile lift of {d1_lift}× over contacting people at random. That
