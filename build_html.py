...
body {{ background:#fff; }}
.article {{ max-width: 760px; margin: 0 auto; padding: var(--space-12) var(--space-6) var(--space-16); }}

/* type scale: one body size, one small (caption) size, deliberate headings */
.article p {{ font-size: 1.0625rem; line-height: 1.72; color: var(--text); margin: 0 0 1.15rem; }}
.article h1 {{ font-size: 2.05rem; line-height: 1.16; font-weight: 700; letter-spacing: -.015em;
  color: var(--text); margin: 0 0 1rem; }}
.article .lead {{ font-size: 1.2rem; line-height: 1.62; color: var(--text-2); margin: 0 0 1.6rem; font-weight: 400; }}
.eyebrow {{ font-size: .78rem; letter-spacing: .09em; text-transform: uppercase; color: var(--muted);
  font-weight: 600; margin: 0 0 1rem; }}

.sec {{ margin-top: 3.4rem; }}
.sec > h2 {{ font-size: 1.4rem; font-weight: 600; letter-spacing: -.01em; color: var(--text);
  margin: 0 0 .3rem; padding-top: 1.6rem; border-top: 1px solid var(--border); }}
.sec > h2 .n {{ color: var(--accent); font-variant-numeric: tabular-nums; margin-right: .55ch; font-weight: 700; }}
.sec-meta {{ font-size: .85rem; color: var(--muted); margin: 0 0 1.5rem; }}

/* figures: no box, just chart + caption */
figure.fig {{ margin: 1.7rem 0; }}
figure.fig .cap-h {{ font-size: .95rem; font-weight: 600; color: var(--text); margin: 0 0 .15rem; }}
figure.fig figcaption {{ font-size: .85rem; color: var(--muted); line-height: 1.55; margin-top: .55rem; }}
.chart {{ width: 100%; }}
...
.bi-kpi-row {{ display: grid; grid-template-columns: repeat(auto-fit, minmax(132px, 1fr)); gap: 1.4rem 1.2rem;
  margin: 1.7rem 0; padding: 1.2rem .2rem; border-top: 1px solid var(--border); border-bottom: 1px solid var(--border); }}
.bi-kpi {{ background: none !important; border: none !important; box-shadow: none !important; padding: 0 !important;
  display: flex; flex-direction: column; gap: 3px; }}
.bi-kpi-label {{ font-size: .78rem; color: var(--muted); font-weight: 500; }}
.bi-kpi-value {{ font-size: 1.5rem; font-weight: 700; color: var(--text); line-height: 1.1; letter-spacing: -.01em; }}
.bi-kpi-sub {{ font-size: .76rem; color: var(--muted); line-height: 1.4; }}
...
.article ul.flow {{ margin: .6rem 0 1.2rem; padding-left: 1.2em; }}
.article ul.flow li {{ font-size: 1.02rem; line-height: 1.6; color: var(--text); margin: .45rem 0; }}
.article ul.flow li b {{ font-weight: 600; }}

/* tables: keep, give them a caption + breathing room */
.tbl-cap {{ font-size: .95rem; font-weight: 600; color: var(--text); margin: 1.7rem 0 .5rem; }}
.bi-table {{ font-size: .92rem; }}

/* segment list: hairline-separated rows, not stacked cards */
.bi-stack {{ display: flex; flex-direction: column; gap: 0; }}
.seg-card.bi-card {{ background: none; border: none; border-top: 1px solid var(--border); border-radius: 0;
  box-shadow: none; padding: .9rem 0; gap: .35rem; }}
.seg-card.bi-card:first-child {{ border-top: none; padding-top: 0; }}
.seg-card h3 {{ font-size: .98rem; font-weight: 600; }}
.seg-card .seg-stat {{ font-size: .86rem; }}
.seg-card .note {{ font-size: .9rem !important; line-height: 1.5; color: var(--text-2) !important; }}
...
<article class="article">
  ...
  <header>
    ...
    <p class="lead">Japan's life-insurance market has stopped growing. Almost nine in ten households are already
      covered, the population is shrinking, and for an insurer like MetLife Japan the value of the business now sits in
      the policies it already holds rather than in the few it can still sell. This is a worked example of how I would
      approach that as a data scientist: rank the in-force book by the chance each policy lapses in the next twelve
      months, work out what is driving that risk, group policyholders into segments a retention team can act on, and put
      a defensible number on what intervening is worth.</p>
  ...
  <section class="sec">
    ...
    <p>The Japanese life market is large and mature. Industry premium income reached ¥37.52 trillion in FY2023, and the
      number of individual policies in force has risen for sixteen straight years. But that growth is shallow: 89% of
      two-or-more-person households already hold cover, the population has been shrinking for over a decade, and the
      birth rate is at a record low. When the pool of new customers is this thin, the economics shift. Each year roughly
      5.9% of in-force benefits lapse or surrender, and every policy that walks out the door takes future premium and
      embedded value with it. Keeping the book is no longer a back-office concern; it is the main way the business
      grows.</p>
    ...
    <p>This is also where MetLife competes. MetLife Insurance K.K. has been in Japan since 1973 (originally as Alico, the
      ...
      through tied agents and bancassurance. Those are exactly the segments where affordability-driven and rate-driven
      lapse bites hardest, so a retention model is not an abstract exercise for them. The question becomes concrete:
      which policies are about to leave, and why?</p>
  </section>
  ...
  <section class="sec">
    ...
    <p>I framed lapse as a binary prediction: for each in-force policy, what is the probability it terminates within the
      ...
      split.</p>
    <p>The honest result is that they tie. The boosted model averages {cv_gbm} AUC across folds; the logistic model
      averages {cv_log}. That difference is {gap_word}. The engineered non-linear interactions buy almost nothing here,
      which is a useful finding in itself: for a regulated insurer, a transparent GLM that performs as well as a black
      box is the one you would actually ship.</p>
    ...
    <p>The drivers line up with the actuarial literature, and crucially none of the top ones are demographics you cannot
      change. They are operational signals a retention team can act on:</p>
    <ul class="flow" id="driverNotes"></ul>
    <p>To avoid relying on a single attribution method, I also computed SHAP values, which decompose each prediction into
      per-feature contributions. The two methods agree on the cast of important features even if they rank them slightly
      differently, and SHAP has the advantage of explaining individual policies, not just the model as a whole.</p>
    <div class="fig-2">
      <figure class="fig">
  ...
  <section class="sec">
    ...
    <p>Because the scores rank well, risk is concentrated. Contacting the riskiest 20% of the book reaches {cap2} of all
      the lapses that are going to happen, with a top-decile lift of {d1_lift}x over contacting people at random. That
      concentration is what makes targeted outreach economic instead of spraying the whole book.</p>
    <div class="fig-2">
      <figure class="fig">
  ...
  <section class="sec">
    ...
    <p>Ranking tells you <i>who</i> to call first. Segmentation tells you <i>what to say</i>. Grouping policyholders by
      ...
      k-means and chose five segments for actionability, one play each. I am not pretending these are clean natural
      clusters: the silhouette score at k=5 is modest ({sil5:.2f}) and the segments overlap, so they are management
      lenses, not hard partitions. The pattern that matters survives that caveat.</p>
    <div class="split">
      <figure class="fig">
        <div class="cap-h">Value vs risk</div>
        <div class="chart" id="cSeg" style="height:360px"></div>
      ...
      <div class="bi-stack" id="segCards"></div>
    </div>
  </section>
  ...
  <section class="sec">
    ...
    <p>A model only earns its keep if the intervention it enables pays for itself. The logic is simple to state: score
      the book, target the riskiest two deciles, spend a fixed amount per policyholder on outreach and friction removal
      (an auto-pay conversion, a flexible payment plan, a service call), and assume that effort cuts lapse among those
      contacted by a modest relative amount. The value credited is only the lifetime value of the policies that would
      actually have lapsed, not the average of everyone contacted, so the gain is not inflated by the roughly 80% of the
      targeted group who were never going to leave.</p>
    ...
    <p>The honest reading is that this programme is worth doing across a wide range of assumptions, but not all of them.
      Presenting the sensitivity grid rather than a single ROI figure is the difference between a number a committee can
      challenge and one it has to take on faith.</p>
  </section>
  ...
  <section class="sec">
    ...
    <p>A retention flag decides who gets attention and who is left alone, so it is worth checking whether it behaves
      evenly across groups before anyone acts on it. I broke the flag down by age band and household income, comparing
  ...
  <section class="sec">
    ...
    <p>Everything above runs on my laptop. Making it useful inside MetLife means putting it on a cadence and into the
      hands of the people who talk to customers. I want to be clear about the line between what is built here and what is
      a proposal: the SHAP explanations and the fairness audit above are implemented; the Azure and MLOps plumbing below
      is how I would productionise it, not something I have stood up.</p>
    <div class="fig-2">
      <figure class="fig">
